...
namespace MetaPackage
{
  ...
  public class UpgradableChestReward : BaseReward
  {
    public UpgradableChestReward(RewardSettings settings) : base(settings)
    { }

    public List<ChestRewardResult> claimedRewards = new();

    public override void Claim()
    {
      UpgradeChestSettings upgradeChestSettings = MetaManager.Instance.GetUpgradeChestSettings(settings.upgradeChestKind);
      var rarityChestDataByRarity = upgradeChestSettings.rarityUpgradeChestDatas.ToDictionary(x => x.rarity, x => x);

      var nbRewardsByKind = new Dictionary<Enum, int>();
      ...
      foreach (var rarityChestData in upgradeChestSettings.rarityUpgradeChestDatas)
      {
